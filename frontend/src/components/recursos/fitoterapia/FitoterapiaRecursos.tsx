...
const LeafPlaceholder = ({ color }: { color: string }) => (
  <Flex direction="column" align="center" gap={2} opacity={0.5}>
    <svg width="44" height="44" viewBox="0 0 44 44" fill="none">
      <path d="M22 4 C13 11 9 22 12 33 C19 36 31 30 36 20 C40 11 33 4 22 4 Z" fill={color}/>
      <path d="M22 4 C22 18 20 28 12 33" stroke="white" strokeWidth="1.5" fill="none" opacity="0.4"/>
    </svg>
    <Text color={color} fontSize="xs" fontStyle="italic" textAlign="center">Foto próximamente</Text>
  </Flex>
);
...
const FotoGaleria = ({ planta }: { planta: Planta }) => {
  ...
  return (
    <Box mb={5}>
      <Box
        w="100%" h={{ base: "210px", md: "270px" }}
        borderRadius="xl" overflow="hidden"
        bg={MODAL_COLOR + "18"} border={`1px solid ${MODAL_COLOR}44`}
        display="flex" alignItems="center" justifyContent="center"
        mb={fotos.length > 1 ? 3 : 0}
      >
        {!errores[active] ? (
          <Image src={fotos[active]} alt={planta.nombre} w="100%" h="100%" objectFit="cover" onError={() => markError(active)} />
        ) : (
      ...
      {fotos.length > 1 && (
        <Flex gap={2} flexWrap="wrap">
          {fotos.map((f, i) => (
            <Box
              key={i} w="58px" h="58px" borderRadius="lg" overflow="hidden"
              cursor="pointer"
              border={i === active ? `2.5px solid ${MODAL_COLOR}` : "2px solid rgba(0,0,0,0.08)"}
...
═══════════════════════════════════════════ */
const AudioPlayer = ({ accentBg, accentBorder }: {
  textMid: string; accentBg: string; accentBorder: string;
}) => (
  <Flex justify="center" mb={7}>
    <Box w="100%">
      <Box
        bg={accentBg} border={`1px solid ${accentBorder}`}
        borderRadius="xl" px={4} py={3}
        display="flex" justifyContent="center"
      >
        <audio controls style={{ width: "100%", accentColor: MODAL_COLOR }}>
          {/* src se añadirá cuando el audio esté disponible */}
        </audio>
      </Box>
    </Box>
  </Flex>
    ...
    >
      ...
      >
        ...
        <Box position="relative" zIndex={2} px={{ base: 6, md: 10 }} pt={10} pb={10}>
          ...
          {/* AUDIO */}
          <AudioPlayer textMid={textMid} accentBg={accentBg} accentBorder={accentBorder} />

...
// /metodo/nutricion/prediabetes. Mismo patrón que ComicIntegralModal.
// ─────────────────────────────────────────────────────────────────────────
interface ComicDiabetesModalProps {
  isOpen: boolean;
  onClose: () => void;
  onContinue: () => void;
}
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size="full" isCentered scrollBehavior="outside">
      ...
      <ModalContent bg="transparent" border="none" borderRadius="0" boxShadow="none" m={0}
        ...
        {/* Botón «Ir al test →» fijo, a la IZQUIERDA de la X del ComicViewer. */}
        <Box as="button" onClick={onContinue} position="fixed" top={{ base: 3, md: 5 }}
          ...
          {/* Fondo imagen + velo, como el resto de botones de cómic de Nutrición. */}
          ...
          <Box as="span" position="relative" zIndex={1} display={{ base: "none", md: "inline" }}>{t("metodo.irA", { destino: t("metodo.destino.test") })}</Box>

...
// la historia, sino de que se ENTIENDA, y en el menor número de párrafos que
// aguante la idea. El esquema de cada momento es:
//     [pregunta gancho]  →  cuerpo (los párrafos que hagan falta, no más)
//                        →  «Dato curioso: …»  [ →  «Dato curioso II: …»]
// Lo único obligatorio es el cuerpo. La pregunta gancho va vacía (`""`) en todos
// los momentos menos uno —se probaron y se quitaron—, y el dato curioso solo se
// pone cuando hay algo curioso que contar: mejor ninguno que uno flojo. Caben dos
// como máximo, y el segundo se escribe empezando por «Dato curioso II: » (ver el
// tipo `Datos`). Las eras NO tienen texto de intro: no se pintaba y se borró.
...
// hasta dos datos curiosos, OPCIONALES) y, opcionalmente, páginas «Profundiza»
// detrás. Los datos curiosos se pueden omitir sin más: si el momento no tiene
// ninguno pero sí «Profundiza», se pasan los extras directamente en su lugar
// (sin `undefined` de relleno).
const hito = (
  era: string,
  key: string,
  titulo: string,
  ...
  pregunta: string,
  cuerpo: string[],
  datosOExtras?: Datos | Profundiza[],
  masExtras?: Profundiza[],
): SubHito => {
  ...
  },
  {
    ...
    subhitos: [
      ...
      hito("edad-antigua", "unificacion-egipto", "Unificación de Egipto", "≈3100 a. C.",
        "¿Por qué existían un Alto y un Bajo Egipto?",
        [

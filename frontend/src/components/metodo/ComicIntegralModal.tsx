...
// ─────────────────────────────────────────────────────────────────────────
// Cómic de transición «Lo integral». Va ENTRE la página de El hambre y el plato
// de Harvard: se abre al pulsar «Crea tu plato →» y termina navegando a
// /metodo/nutricion/plato. Mismo patrón que ComicHambreModal.
// ─────────────────────────────────────────────────────────────────────────
interface ComicIntegralModalProps {
  isOpen: boolean;
  onClose: () => void;
  onContinue: () => void;
}

export function ComicIntegralModal({ isOpen, onClose, onContinue }: ComicIntegralModalProps) {
  const t = useT();
    ...
    <Modal isOpen={isOpen} onClose={onClose} size="full" isCentered scrollBehavior="outside">
      ...
      <ModalContent bg="transparent" border="none" borderRadius="0" boxShadow="none" m={0}
        ...
        <Box as="button" onClick={onContinue} position="fixed" top={{ base: 3, md: 5 }}
          ...
          <Box as="span" position="relative" zIndex={1} display={{ base: "none", md: "inline" }}>{t("metodo.irA", { destino: t("metodo.destino.plato") })}</Box>

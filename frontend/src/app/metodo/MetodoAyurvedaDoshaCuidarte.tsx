...
// Fila de sabor (favorable = check; a moderar = aviso).
...
// Fila seleccionable de compromiso (selección única).
...
export default function MetodoAyurvedaDoshaCuidarte() {
  ...
  const irSiguiente = () => {
    ...
    navigate(`/metodo/ayurveda/dosha/${doshaKey}/cursos`);
  };
          ...
          <Flex direction="column" align="center" w="100%" maxW="640px" gap={6}>
            ...
            <Panel color={meta.color}>
              ...
              <Text color={`${TINTA}cc`} fontSize={{ base: "md", md: "lg" }} textAlign="center" lineHeight="1.8">
                Tu alimentación y estilo de vida para {meta.label} estará disponible muy pronto.
              </Text>
            </Panel>
          </Flex>
        </Flex>
      ...
      <Flex flex="1" justify="center" px={{ base: 5, md: 10, lg: 16 }} pt={{ base: 8, md: 12 }} pb={{ base: 14, md: 20 }}>
        <Flex direction="column" align="center" w="100%" maxW="820px" gap={{ base: 6, md: 7 }}>
          ...
          <MetodoStepHeader
            ...
            nom={ayurvedaNom}
            mb={0}
            prev={{ label: "← Equilibrio", onClick: () => { void persist(reflexion, compromiso); navigate(`/metodo/ayurveda/dosha/${doshaKey}/desequilibrio`); } }}
            extra={ilustracionesBtn}
            next={{ label: "Cursos →", onClick: irSiguiente, icon: guardado ? undefined : <Lock size={14} /> }}
          />
          ...
          <Box ref={reflexionRef} w="100%">
            <Panel color={meta.color}>
              ...
              {/* Mi compromiso (selección única) — solo si la dosha lo define */}

...
export const NUTRIENTES: Nutriente[] = [
  ...
  },
  {
    ...
    label: "Drogas",
    color: "#7a8794",
    img: "/recorrido/nutricion/portadas/drogas.webp",
    resumen: "Moléculas que no alimentan: ocupan cerraduras.",
    descripcion: [
      "Esto no es un nutriente, y por eso está aquí. Un nutriente entra, se rompe y se convierte en ti: en membrana, en enzima, en energía. Una droga entra, se sienta en una cerradura que ya tenías, deja un mensaje que tu cuerpo no había pedido y se va. Nada de ella se queda formando parte de ti; lo único que queda es la factura de haberla sacado.",
      "Casi ninguna inventa nada nuevo, y eso es lo fascinante: funcionan porque se parecen a algo tuyo. La nicotina encaja en los receptores de tu acetilcolina; el THC, en los de tus propios cannabinoides; la cafeína, en los de tu adenosina; el ibuprofeno bloquea una de tus enzimas. Son llaves falsas para cerraduras auténticas. Por eso funcionan tan bien y por eso descolocan tanto.",
      "Y ahí está la clave de por qué degradan poco a poco. Tu célula no es pasiva: cuando un mensaje llega demasiado fuerte y demasiadas veces, se defiende cambiando su maquinaria. Retira receptores, o fabrica más, o ajusta la enzima que los limpia. Eso es la tolerancia: la misma dosis ya no hace lo mismo. Y también es la abstinencia, porque el sistema ya está calibrado para funcionar CON la sustancia, y sin ella queda descompensado. A eso se suma la factura silenciosa: el hígado gastando su maquinaria de detoxificación y sus antioxidantes en cada dosis, y el estrés oxidativo y la inflamación acumulándose en tejidos que no duelen. El cuerpo no lleva la cuenta en síntomas: la lleva en tejido.",
      "Nada de esto es un juicio moral, y algunas de estas moléculas son medicamentos excelentes. Es simplemente entender qué pasa dentro cuando entran, porque comprender el mecanismo es lo que convierte una decisión automática en una decisión.",
    ],
    tipos: [
      { nombre: "Antiinflamatorios y analgésicos", desc: "Ibuprofeno, paracetamol. Bloquean enzimas tuyas." },
      { nombre: "Cannabinoides", desc: "THC: ocupa los receptores de tus propios endocannabinoides." },
      { nombre: "Nicotina (humo y aerosol)", desc: "Tabaco y vaper: la misma llave, distinto vehículo." },
      { nombre: "Estimulantes cotidianos", desc: "Cafeína: no te da energía, te tapa el sueño." },
    ],
    queHacen: [
      "Ocupan receptores y enzimas propios: son llaves falsas de cerraduras reales.",
      "La célula se recalibra (tolerancia) y sin la sustancia queda descompensada.",
      "El hígado, el riñón y tus antioxidantes pagan la factura de cada dosis.",
    ],
    donde: ["Farmacia sin receta", "Tabaco y vapeadores", "Cannabis", "Café, té y energéticas", "Alcohol (ver Etanol)"],
    tarjetas: [
      {
        key: "ibuprofeno",
        titulo: "Ibuprofeno",
        ...
        parrafos: [
          ...
          "El ibuprofeno se mete en el hueco donde la COX agarra al ácido araquidónico y lo bloquea. Sin COX no hay prostaglandinas, y sin prostaglandinas no hay dolor, ni fiebre, ni hinchazón. Fíjate en lo que NO ha pasado: el daño sigue exactamente donde estaba. Solo has silenciado el mensajero que te lo estaba contando.",
          "Y aquí llega el precio. La COX-2 es la que se dispara cuando hay inflamación, pero la COX-1 está encendida todo el día haciendo tareas de mantenimiento que nadie ve. Fabrica la prostaglandina que mantiene la capa de moco y bicarbonato que protege tu estómago de su propio ácido, la que mantiene el flujo de sangre dentro del riñón cuando hay poca agua, y la que las plaquetas usan para coagular. El ibuprofeno no distingue: bloquea las dos.",
          "En el estómago, eso significa mucosa sin escudo. Aparecen gastritis, erosiones y úlceras, y lo peligroso es que muchas veces avisan poco o nada: el mismo fármaco que está pelando la mucosa es el que apaga el dolor que te habría alertado. Se toma con comida no porque «proteja», sino porque parte del daño es por contacto directo; el resto llega por la sangre y ocurre igual.",
          "En el riñón, esas prostaglandinas son las que mantienen abiertas las arteriolas que llevan sangre al filtro. Si estás deshidratado, has sudado mucho en una carrera larga o tomas ciertos fármacos para la tensión, el riñón depende de ellas para no quedarse sin flujo. Quitarlas justo ahí es el escenario clásico de fallo renal agudo por antiinflamatorios, y es una de las razones por las que no es buena idea el ibuprofeno «preventivo» antes de una maratón.",
          "Con el uso repetido durante años, la cuenta se va sumando en cosas discretas: retención de sodio y agua (la tensión sube unos puntos y los fármacos para bajarla funcionan peor), pérdida progresiva de función renal, y en el intestino delgado un aumento de la permeabilidad de la barrera que la microbiota nota. En dosis altas y mantenidas también se asocia a más riesgo cardiovascular.",
          "Nada de esto lo convierte en un enemigo: es un medicamento espléndido y a veces imprescindible. Lo que conviene mirar de frente es el «por si acaso» diario, la caja en el bolso, el ibuprofeno tomado cada tarde durante meses. Un dolor que vuelve una y otra vez es una pregunta que hay que responder, no una señal que haya que apagar.",
        ],
      },
      {
        key: "paracetamol",
        ...
        parrafos: [
          ...
          "La mayor parte del paracetamol se «etiqueta» para eliminarlo: el hígado le pega una molécula (un glucurónido o un sulfato) que lo vuelve soluble, y se va por la orina sin más. Pero una pequeña fracción, en torno a un 5-10%, la coge otra enzima, el CYP2E1, y la oxida a NAPQI: un metabolito extremadamente reactivo capaz de pegarse a las proteínas de la célula y destrozarlas.",
          "Tienes un extintor para eso. El glutatión, el antioxidante maestro de tus células, neutraliza el NAPQI en cuanto aparece. Con una dosis normal, el extintor va sobrado y no notas nada. El problema es que el glutatión es un depósito limitado y hay tres formas de quedarse sin él: pasarse de dosis, tener el CYP2E1 acelerado (el alcohol crónico lo induce, así que fabrica MÁS NAPQI de lo normal), o llegar con el depósito bajo (ayuno prolongado, desnutrición, enfermedad). Cuando el glutatión se agota, el NAPQI queda libre y empieza a matar hepatocitos.",
          "De ahí que el paracetamol sea, en muchos países, la primera causa de fallo hepático agudo por medicamentos. Y casi nunca por querer: la franja entre la dosis útil y la dosis peligrosa es más estrecha de lo que la gente cree, y las dosis se suman sin darte cuenta, porque muchos antigripales y analgésicos combinados ya lo llevan dentro. Dos «cosas distintas» de la farmacia pueden ser el mismo fármaco dos veces.",
          "El detalle que más me gusta contar: el antídoto en urgencias es N-acetilcisteína, que es materia prima para fabricar glutatión. No se ataca al fármaco, se le rellena el extintor a la célula. Y aunque una dosis normal no te haga daño alguno, entender esto cambia cómo lo miras: cada gramo se paga en moneda antioxidante, la misma con la que tu hígado se defiende del resto de cosas de la vida.",
        ],
      },
      {
        key: "marihuana",
        ...
        parrafos: [
          ...
          "El THC encaja en el mismo receptor, el CB1, y rompe las tres reglas de golpe. No lo fabrica una neurona concreta: llega por la sangre e inunda el cerebro entero a la vez. No es local: baja el volumen en todas partes, también donde nadie lo había pedido. Y no se apaga en segundos: se queda horas. No es un mando de volumen, es un dedo apoyado en el botón.",
          "Todo lo que se siente sale del mapa de donde está el CB1, que no está repartido al azar. Hay mucho en el hipocampo, la fábrica de memoria nueva: de ahí que cueste retener lo que acaba de pasar. Hay mucho en la corteza prefrontal, la que planifica y frena impulsos: de ahí la desinhibición y la dificultad para sostener un hilo. Hay mucho en el cerebelo y los ganglios basales, que coordinan el movimiento: de ahí la torpeza y el tiempo de reacción largo, que es lo que hace peligroso conducir. Y hay en el hipotálamo, donde el sistema regula el hambre: de ahí el hambre repentina, que además pasa por activar las mismas neuronas que normalmente dicen «ya has comido bastante».",
          "Y ahora la degradación lenta. Al recibir ese mensaje desmedido una y otra vez, la neurona se protege quitando receptores CB1 de su superficie. Es un cambio físico y medible: en estudios de imagen cerebral se ve la caída en consumidores habituales. Dos consecuencias. La primera, tolerancia: hace falta más para lo mismo. La segunda, y más importante, es que ahora tu propio sistema tiene menos donde encajar, y ese sistema era el que regulaba tu apetito, tu sueño y tu ansiedad de base. Por eso al dejarlo aparecen irritabilidad, insomnio, sueños vívidos, falta de hambre y un malestar difuso: no es «flojera», es un sistema descalibrado buscando su punto. La buena noticia es que los receptores se recuperan en unas semanas de abstinencia.",
          "Hay un momento en que esto pesa mucho más: el cerebro en desarrollo. La maduración de la corteza prefrontal y la poda de conexiones sobrantes siguen hasta pasados los veinte años, y el sistema endocannabinoide es una de las señales que guían esa obra. Alterarlo mientras se está construyendo no es lo mismo que alterarlo cuando ya está hecho: el consumo intenso y temprano se asocia a peor memoria verbal y atención, y a un riesgo mayor de psicosis en personas con predisposición. Y la dosis importa más que nunca, porque el cannabis actual concentra varias veces más THC que el de hace unas décadas: la planta «de siempre» ya no es la misma planta.",
          "Dos detalles que se pasan por alto. Uno: fumada, es humo. Combustión significa alquitranes, monóxido de carbono, irritación bronquial y tos crónica, con independencia de que la planta sea natural; los pulmones no distinguen el origen del humo. Y dos: el THC es tan liposoluble que se guarda en el tejido graso y se va soltando despacio durante días o semanas. Por eso se detecta mucho después, y por eso el cuerpo tarda en volver del todo a su sitio.",
        ],
      },
      {
        key: "tabaco",
        ...
        parrafos: [
          ...
          "Y la célula responde justo al contrario de lo que se espera: fabrica MÁS receptores. Al haber siempre nicotina ocupándolos, el cerebro sube el número para mantener el equilibrio. Cuando bajan los niveles, todos esos receptores extra se quedan vacíos, y ese hueco es exactamente lo que se siente como ansia. Ahí está la trampa: el cigarro de la mañana no da placer, solo apaga una abstinencia de ocho horas de sueño. Se fuma para volver a la normalidad de quien no fuma.",
          "Del humo, lo primero que actúa es el monóxido de carbono. Se pega a la hemoglobina entre 200 y 250 veces mejor que el oxígeno, así que un porcentaje de tus glóbulos rojos deja de transportar oxígeno y se dedica a pasear monóxido. Todo el cuerpo respira un poco peor: el músculo rinde menos, la piel se nutre peor, las heridas cierran más despacio.",
          "Después está el endotelio, la capa de una célula de grosor que recubre por dentro todas tus arterias. El humo la castiga directamente: radicales libres, menos óxido nítrico disponible (la molécula con la que la arteria se relaja), más adherencia de plaquetas. Resultado: arterias más rígidas, más placa, más tendencia a formar coágulos. De ahí el infarto, el ictus, las piernas que duelen al andar y la disfunción eréctil, que suele ser el primer aviso de un problema vascular general.",
          "En el pulmón, el daño empieza por la limpieza. Tus vías respiratorias están tapizadas de cilios, pelillos que barren el moco hacia arriba con todo lo que ha quedado atrapado. El humo los paraliza y luego los destruye, así que el moco se queda dentro: por eso la tos y por eso las infecciones se repiten. Más adentro, el humo desequilibra la balanza entre las enzimas que degradan tejido y las que lo protegen, y los alveolos, esas bolsitas donde entra el oxígeno, se van rompiendo. Eso es el EPOC, y a diferencia de casi todo lo demás, no se recupera: el alveolo perdido no vuelve.",
          "Y en el ADN, la parte más lenta y más silenciosa. Carcinógenos como el benzopireno y las nitrosaminas no dañan directamente: el propio hígado los transforma, intentando eliminarlos, en moléculas reactivas que se pegan al ADN formando aductos. Tus sistemas de reparación arreglan la inmensa mayoría. Pero cada calada es otra tirada de dados, y basta que un error caiga en el gen equivocado y sobreviva. Por eso el cáncer de pulmón no es cuestión de mala suerte ni de justicia, sino de probabilidad acumulada durante años.",
          "Luego hay un desgaste callado del que se habla poco: el humo consume vitamina C y antioxidantes a un ritmo altísimo, activa las enzimas que degradan el colágeno de la piel (de ahí las arrugas finas y ese envejecimiento cutáneo tan característico), empeora la densidad ósea, adelanta la menopausia, dificulta la cicatrización de cualquier cirugía y aumenta la resistencia a la insulina. No hay un solo sistema del cuerpo que quede al margen.",
          "Y precisamente por eso merece la pena el final: casi todo mejora al dejarlo, y antes de lo que parece. El monóxido se aclara en horas. El gusto y el olfato vuelven en días. Los cilios se regeneran en semanas y meses (esa tos que aparece al dejar de fumar es el pulmón limpiando por fin). El riesgo cardiovascular cae cerca de la mitad en el primer año, y el de cáncer sigue bajando durante décadas. El cuerpo no lleva rencor: en cuanto dejas de darle motivos, se pone a reparar.",
        ],
      },
      {
        key: "vaper",
        ...
        parrafos: [
          ...
          "Y el aerosol tampoco es vapor de agua. El propilenglicol y la glicerina son inocuos en un jarabe, pero al calentarse en una resistencia se descomponen en carbonilos: formaldehído, acetaldehído, acroleína. Son irritantes de la vía aérea y algunos están clasificados como cancerígenos. La cantidad no es fija: sube con la potencia del dispositivo y se dispara cuando la mecha está seca (el «dry hit», ese sabor a quemado). A eso se suman metales que el propio aerosol arranca de la resistencia y las soldaduras: níquel, cromo, estaño, plomo. Nada de eso venía en la etiqueta del líquido.",
          "Dentro de la célula, lo que se ve en los estudios de epitelio bronquial es coherente con eso: estrés oxidativo, respuesta inflamatoria, alteración del batido de los cilios (otra vez la escoba que limpia el pulmón) y una defensa peor frente a virus y bacterias. En el sistema vascular se ha medido rigidez arterial y disfunción del endotelio ya tras una sola sesión de vapeo. Menos daño que el humo, sí; cero, no.",
          "Los aromas, que parecen el detalle más inocente, son de lo menos estudiado. El diacetilo, que da el sabor mantecoso y dulce, se asoció a bronquiolitis obliterante (una cicatrización irreversible de los bronquiolos más finos) en trabajadores de fábricas de palomitas que lo respiraban. El cinamaldehído y el mentol alteran la función ciliar y la de los macrófagos que patrullan el alveolo. El punto es sencillo: una molécula puede ser perfectamente segura comida y comportarse de otra forma calentada a 250 grados e inhalada. Son dos preguntas distintas y solo se ha contestado una.",
          "Conviene recordar lo de 2019, cuando aparecieron cientos de casos de daño pulmonar agudo grave, el llamado EVALI, en su mayoría por acetato de vitamina E usado como espesante en cartuchos ilegales de THC. La vitamina E es un antioxidante excelente. Inhalada, era un aceite recubriendo el alveolo. Es la mejor lección disponible sobre lo que ocurre al inhalar sustancias cuyo comportamiento en el pulmón nadie había estudiado.",
          "El resumen honesto tiene dos partes que no se contradicen. Como herramienta para dejar de fumar, con acompañamiento y con fecha de salida, tiene evidencia a favor y es preferible al tabaco. Como producto de inicio, es un dispositivo de dependencia muy eficaz que usa el pulmón como campo de pruebas, y del que nadie puede decir qué se verá a treinta años, por la razón más simple de todas: aún no han pasado treinta años.",
        ],
      },
      {
        key: "cafeina",
        ...
        parrafos: [
          ...
          "La cafeína se parece tanto a la adenosina que encaja en su receptor, pero no lo activa: solo lo ocupa. Es un tapón. La adenosina sigue ahí, sigue subiendo, sigue avisando, y el mensaje no entra. No estás menos cansado: estás igual de cansado sin poder oírlo.",
          "Y al quitar ese freno pasan varias cosas de golpe. Los sistemas de dopamina y noradrenalina, que la adenosina mantenía a raya, suben un punto: llega el foco, el ánimo, algo de tensión arterial y ese ligero temblor si te has pasado. También se estrechan un poco los vasos del cerebro, que es la razón por la que la cafeína ayuda en algunas migrañas y aparece en varios analgésicos, y también por la que duele la cabeza cuando la dejas de golpe y esos vasos se dilatan de nuevo.",
          "La adaptación llega en menos de dos semanas y es de manual: si sus receptores están siempre tapados, la célula fabrica más receptores de adenosina. Ahora hace falta más cafeína para tapar los mismos avisos y, sobre todo, sin cafeína hay más receptores libres de lo normal para una adenosina que sigue acumulándose igual. Traducido: el café ya no te sube por encima de tu línea base, solo te devuelve a ella. Ese es el momento exacto en que alguien empieza a decir «yo sin café no soy persona». No es una exageración: es un sistema que ya solo funciona en equilibrio con la sustancia.",
          "Luego está el reloj, que es donde se hace el daño real. La vida media de la cafeína es de unas cuatro a seis horas, y varía mucho: los anticonceptivos y el embarazo la alargan bastante, y el tabaco la acorta (por eso quien deja de fumar y mantiene sus cafés de siempre se encuentra de pronto pasadísimo de cafeína). Con seis horas de vida media, un café de las cinco de la tarde conserva la mitad de su dosis en sangre a medianoche. Puedes dormirte perfectamente y aun así tener menos sueño profundo, que es el que repara. Y ahí se cierra el círculo: duermes peor, amaneces más cansado, necesitas más café.",
          "Y la deuda no desaparece: la adenosina que has estado tapando durante horas sigue acumulada, esperando. Cuando la cafeína se va, entra todo el mensaje de golpe. Eso es el bajón de media tarde.",
          "Con las cantidades, la referencia habitual para un adulto sano son unos 400 mg al día, más o menos tres o cuatro cafés, y bastante menos en el embarazo. El capítulo aparte son las bebidas energéticas: dosis altas mezcladas con azúcar, consumidas rápido y a menudo por adolescentes, es un escenario distinto al de una taza de café.",
          "Un matiz justo para terminar: el café en sí, como bebida, tiene polifenoles y sus asociaciones con la salud son más bien favorables. El problema no es el grano. Es usarlo para tapar una deuda de sueño que, tarde o temprano, hay que pagar durmiendo.",
        ],
      },
    ],
  },
